...
class LinearFuse(nn.Module):
    def __init__(self, ndim):
        super().__init__()
        self.linear = nn.Linear(4 * ndim, 2 * ndim)
...
class NCSNpp(nn.Module):
    ...
    def __init__(
        ...
        fourier_scale=16,
        image_size=256,
        flow_type="normal", # "normal" / "mean"
        fuse_type="add", # add / linear
        ...
        **unused_kwargs,
        ...
        '''
        print("### NCSNpp init ###")
        print("image_size:", image_size, flush=True)
        print("conditional:", conditional, flush=True)
        print("flow_type:", flow_type, flush=True)
        print("fuse_type:", fuse_type, flush=True)
        '''
        ...
        assert flow_type in ["normal", "mean"]
        assert fuse_type in ["add", "linear"]
        self.flow_type = flow_type = flow_type.lower()
        self.fuse_type = fuse_type = fuse_type.lower()
        ...
        if flow_type == "mean":
            if fuse_type == "linear":
                self.t_r_fuse = LinearFuse(embed_dim)
            elif fuse_type == "add":
                self.t_r_fuse = AddFuse()
            else:
                raise ModuleNotFoundError("unknown fuse type!")
    ...
    def forward(self, xt, y, r=None, t=None):
        ...
        if self.embedding_type == "fourier":
            ...
            temb = modules[m_idx](used_sigmas) # torch.log(used_sigmas)
            if self.flow_type == "mean":
                remb = modules[m_idx](r_cond) # torch.log(r_cond)
            m_idx += 1
        ...
        if self.conditional:
            ...
            if self.flow_type == "mean":
                remb = modules[m_idx](remb)
            m_idx += 1
            temb = modules[m_idx](self.act(temb))
            if self.flow_type == "mean":
                remb = modules[m_idx](self.act(remb))
            m_idx += 1
            if self.flow_type == "mean":
                fuse_emb = self.t_r_fuse(temb, remb)
                assert fuse_emb.shape == temb.shape
                temb = fuse_emb
        else:
            temb = None
